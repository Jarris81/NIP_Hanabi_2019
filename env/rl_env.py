...
class HanabiEnv(Environment):
    ...
    def step(self, action):
        ...
        last_player = self.state.cur_player()
        ...
        fireworks = observation['player_observations'][last_player]['fireworks']

        # append to history of env
        last_moves = observation['player_observations'][last_player]['pyhanabi'].last_moves()
        for stack in self.history_stack:
            stack.append((last_player, last_moves[len(last_moves)-1]))

        should_get_rewarded = None
        #should_get_rewarded = self.custom_reward(last_player, fireworks)
        done = self.state.is_terminal()
        # Reward is score differential. May be large and negative at game end.
        reward = self.state.score() - last_score
        info = {"should_get_rewarded": should_get_rewarded}

        return (observation, reward, done, info)

    def custom_reward(self, last_player, fireworks):

        # get the last action and the player
        print("------------------------Getting Custom Reward -------------------------------------")
        # list for players who should get rewarded
        should_get_rewared = list()

        # get player history stack
        player_history_stack = self.history_stack[last_player]
        last_player, last_history_move = player_history_stack[len(player_history_stack) - 1]
        last_move = last_history_move.move()

        really_helpful = False

        if (last_move.type() is pyhanabi.HanabiMoveType.PLAY and last_history_move.scored()) or \
                (last_move.type() is pyhanabi.HanabiMoveType.DISCARD and
                 fireworks[pyhanabi.color_idx_to_char(last_history_move.color())] != last_history_move.rank()):

            # check if someone hinted that card
            for helpful_player, helpful_history_move in player_history_stack:

                # get HanabiMove object
                helpful_move = helpful_history_move.move()

                if (helpful_player + helpful_move.target_offset()) % self.players == last_player and\
                        last_history_move.move().card_index() in helpful_history_move.card_info_revealed() and\
                        (helpful_move.type() == pyhanabi.HanabiMoveType.REVEAL_COLOR or
                         helpful_move.type() == pyhanabi.HanabiMoveType.REVEAL_RANK):

                    # check when the helpful hint was given
                    idx_helpful_history_move = player_history_stack.index((helpful_player, helpful_history_move))

                    really_helpful = True

                    print(player_history_stack)
                    print("hint was helpful!!!")
                    print("Helpful hint was:")
                    print(helpful_history_move)
                    print("from helpful player: ", helpful_player)
                    print("Index of that move: ", idx_helpful_history_move)
                    print(
                        "Move was {} turns away: ".format(len(player_history_stack) - 1 - idx_helpful_history_move))
                    print("for the move:")
                    print(last_move)
                    print("to player:", last_player)

                    for discard_player, discard_history_move in player_history_stack[idx_helpful_history_move + 1:-1]:

                        # get HanabiMove object
                        discard_move = discard_history_move.move()

                        # check if between helpful hint and playing card a card at
                        # same slot/index was played or thrown of
                        if discard_player == last_player and\
                                discard_move.card_index() == last_move.card_index() and\
                                (discard_move.type() == pyhanabi.HanabiMoveType.DISCARD or
                                 discard_move.type() == pyhanabi.HanabiMoveType.PLAY):
                            # card got removed, therefore hint was not helpful
                            #print("Helpful hint was not helpful, since card go thrown "
                                  #"away between hint and playing card")
                            really_helpful = False
                            break

                    if really_helpful:
                        turns_after_helpful_hint = len(player_history_stack) - 1 - idx_helpful_history_move
                        ...
                        print("from helpful player: ", helpful_player)
                        print("Index of that move: ", idx_helpful_history_move)
                        print("Move was {} turns away: ".format(len(player_history_stack) - 1 - idx_helpful_history_move))
                        print("for the move:")
                        print(last_move)
                        print("to player:", last_player)
                        ...
                        should_get_rewared.append((turns_after_helpful_hint, helpful_player, reward))

        return should_get_rewared
