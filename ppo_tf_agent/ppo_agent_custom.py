...
from ppo_policy_custom import PPOPolicyCustom
...
@gin.configurable
class PPOAgentCustom(tf_agent.TFAgent):
  """A PPO Agent."""
  ...
  def __init__(self,
    ...
    if normalize_observations:
      self._observation_normalizer = (
          tensor_normalizer.StreamingTensorNormalizer(
              time_step_spec.observation['state'], scope='normalize_observations'))
    ...
    collect_policy = PPOPolicyCustom(
        time_step_spec=time_step_spec,
        action_spec=action_spec,
        actor_network=actor_net,
        value_network=value_net,
    ...
    super(PPOAgentCustom, self).__init__(
        time_step_spec,
        action_spec,
        policy,
        collect_policy,
